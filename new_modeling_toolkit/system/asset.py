...
class Asset(Component, AssetABC):
    ...
    ramp_rate: Annotated[float, Field(gt=0.0)] = Field(
        1.0,
    ...
    td_losses_adjustment: Annotated[float, Field(ge=1.0)] = Field(
        1.0,
    ...
    stochastic_outage_rate: Optional[ts.FractionalTimeseries] = Field(
        default_factory=ts.Timeseries.zero,
        description="Time Dependent - Weather year indexed forced outage rate",
        default_freq="H",
        down_method="mean",
        up_method="interpolate",
        weather_year=True,
    ...
    mean_time_to_repair: Optional[Annotated[float, Field(ge=0.0)]] = Field(None, description="Mean time to repair")
